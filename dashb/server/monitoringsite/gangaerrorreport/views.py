...
def getCommunityAndDebugMode():
        ...
        settingsFile = open(settings.MEDIA_ROOT+'gangaerrorreport/scripts/settings.js', 'r')
...
def download(request, reportId):
        ...
        redirectUrl = '/django/%s/login/%s' % (appName, reportId)
...
def get_reports_JSON(request):
        ...
        reports = Report.objects.order_by('-id').all().filter(is_deleted=False)
...
def report2json(report):
        ...
        downloadLink = "<a href=\"/django/%s/download/%s\">Download</a>" % (appName, str(report.id))
